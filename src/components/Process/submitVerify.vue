  ...
  <el-dialog v-model="dialog.visible" :title="dialog.title" width="50%" draggable :before-close="cancel" center
    ...
    <el-form v-loading="loading" :model="form" label-width="120px">
      ...
      <el-form-item v-if="task.businessStatus === 'waiting'" label="附件">
        <fileUpload v-model="form.fileId" :file-type="['doc', 'xls', 'ppt', 'txt', 'pdf', 'xlsx', 'docx', 'zip']"
                    :file-size="'20'" />
      </el-form-item>
      <el-form-item label="抄送">
        <el-button type="primary" icon="Plus" circle @click="openUserSelectCopy" />
        <el-tag v-for="user in selectCopyUserList" :key="user.userId" closable style="margin: 2px"
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button v-if="task.businessStatus === 'waiting'" :disabled="buttonDisabled" type="primary"
                   @click="openDelegateTask"> 委托 </el-button>
        <el-button v-if="task.businessStatus === 'waiting'" :disabled="buttonDisabled" type="primary"
                   @click="openTransferTask"> 转办 </el-button>
        <el-button
        ...
        <el-button v-if="task.businessStatus === 'waiting'" :disabled="buttonDisabled" type="danger"
                   @click="handleTerminationTask"> 终止 </el-button>
        <el-button v-if="task.businessStatus === 'waiting'" :disabled="buttonDisabled" type="danger"
                   @click="handleBackProcessOpen"> 退回 </el-button>
        <el-button :disabled="buttonDisabled" @click="cancel">取消</el-button>
      </span>
    </template>
    <!-- 抄送 -->
    <UserSelect ref="userSelectCopyRef" :multiple="true" :data="selectCopyUserIds"
                @confirm-call-back="userSelectCopyCallBack"></UserSelect>
    <!-- 转办 -->
...
const handleCompleteTask = async () => {
  ...
  buttonDisabled.value = true;
  try {
    form.value.variables = {...props.variables, ...props.taskVariables};
